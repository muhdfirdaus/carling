...
$boxid = $_GET['box'];
$filename = "box_carling_$boxid";
//create MySQL connection   
$sql=("SELECT bs.sn, bm.box_id, bm.model, bm.qty,bm.status, u.name, FROM_UNIXTIME(bm.timestamp, '%d %M %Y %r') AS ScanTime 
FROM box_sn bs
LEFT JOIN box_master bm ON bs.box_id = bm.box_id
LEFT JOIN USER u ON bm.user_id = u.user_id
WHERE bs.box_id ='$boxid'");
